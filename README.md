...
`mediadog` ist ein leichtgewichtiger, Docker-basierter Hintergrunddienst zur automatisierten Überwachung von Verzeichnissen und zum Verarbeiten/Verschieben von Videodateien via `inotify`[cite: 4, 7].
...
* **Echtzeit-Überwachung:** Reagiert ohne Verzögerung auf Dateisystem-Events (`IN_MOVED_TO`, `IN_CLOSE_WRITE`) mittels `python3-inotify`[cite: 4, 7].
...
* **Filter-Mechanismus:** Ignoriert temporäre Downloads (`.part`, `.ytdl`, `.tmp`), versteckte Dateien sowie ununterstützte Formate[cite: 7].
* **Kollisionsschutz:** Benennt Zieldateien bei Namensgleichheit automatisch um (`video_1.mp4`)[cite: 7].
...
| :--- | :--- | :--- |
| `/media/out` | **Quellverzeichnis** (Eingang für `mediadog`)[cite: 3] | `.mkv`, `.mp4`, `.webm`[cite: 7] |
| `/media/in`[cite: 3, 4] | **Zielverzeichnis** (Ausgang für Folge-Tools)[cite: 3] | `.mkv`, `.mp4`, `.webm`[cite: 7] |
| `/log`[cite: 4] | Log-Dateien (Optional) | Logs / App-Output[cite: 4, 7] |

---

## 📦 Schnellstart mit Docker Compose

Hier ist ein Beispiel für die Integration in eine bestehende `docker-compose.yml`[cite: 3]:

```yaml
services:
  mediadog:
    image: ghcr.io/chaos7x/mediadog:latest
    container_name: mediadog
    hostname: mediadog
    user: "11107:11108"
