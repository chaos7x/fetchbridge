...
Bewusst dasselbe Design wie in yt-upload/tw-recorder (dieselbe Session hat
dort denselben Bug gefixt: eine reine is_dir()-Prüfung auf /log kann ein
echtes Docker-Volume nicht von einem vom Dockerfile per `mkdir -p` fest ins
Image gebackenen Verzeichnis unterscheiden) - hier direkt von Anfang an mit
config._is_dedicated_mount() gebaut, statt denselben Fehler ein drittes Mal
zu machen.
"""
...
def _resolve_log_file_path(app_name: str, explicit_path: str) -> Path:
    ...
    2. /log/<app_name>.log, falls /log als eigenes Docker-Volume gemountet
       ...
       siehe config._is_dedicated_mount())
    ...
    if config._is_dedicated_mount(docker_log_dir):
...
def setup_logging(cfg: dict | None = None, app_name: str = config.APP_NAME) -> None:
    """
    ...
    - Zusätzlich ein rotierender Datei-Handler (max. 10 MB, 5 Backups),
      ...
      Docker-Volume gemountetes /log-Verzeichnis (siehe
      config._is_dedicated_mount() - eine reine is_dir()-Prüfung reicht
      nicht, da das Dockerfile /log auch ganz ohne Mount fest ins Image
      anlegt), oder einen tatsächlich laufenden klassischen Syslog-Daemon.
    ...
    - Das Log-Level selbst wird hier bewusst NICHT gesetzt - das bleibt
      ...
      nicht zu duplizieren oder zu überschreiben.
    - Entfernt zuerst bestehende Handler, damit setup_logging() idempotent
    ...
    docker_log_volume_mounted = config._is_dedicated_mount(Path("/log"))
    ...
    if trigger_reason is not None:
        ...
        reason = "Kein Syslog-Daemon/-Log-Verzeichnis/-Config erkannt - Logging nur nach stdout (journald/docker logs erfasst das bereits)"

    # HTTP-Bibliotheks-Rauschen unabhängig vom eigenen Log-Level drosseln,
    # falls doch mal eine entsprechende Abhängigkeit hinzukommt - fetchbridge
    # selbst nutzt aktuell keine, daher aktuell ein No-Op.
    for noisy_logger_name in ("urllib3", "urllib3.connectionpool", "requests", "httpx"):
        logging.getLogger(noisy_logger_name).setLevel(logging.WARNING)

    logger.info(
        f"Logging initialisiert - aktive Handler: {', '.join(active_handlers_desc)}. {reason}."
    )
